...
resource "null_resource" "cert_manager_ovh_source" {
  provisioner "local-exec" {
    working_dir = "/tmp"
    command     = "curl -L0 https://github.com/baarde/cert-manager-webhook-ovh/tarball/master | tar -xz | mv /tmp/baarde-cert-manager-webhook-ovh-* /tmp/cert-manager-webhook-ovh"
  }
}
