...
export const Error = ({ error = 'default' }: { error: ErrorType | "null" | "undefined" }) => {
  const errors: Record<ErrorType, ErrorView> = {
    ...
    oauthaccountnotlinked: {
      ...
      message: "Your account is not linked to your email. Contact support on Discord",
    },
    oauthcallback: {
  ...
  return <div className="bg-red-400 px-6 py-4 text-white rounded mb-10">
    <h1 className="text-lg mb-2">{name}</h1>
    <p className="text-sm text-red-800 dark:text-red-100 mb-2">
      {message}
    </p>
  </div>
}
