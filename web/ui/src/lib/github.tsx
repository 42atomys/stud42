...
export const getRepo = async () : Promise<Repository> => {
  try {
    const { data } = await axios.get(`https://api.github.com/repos/${PROJECT_PATH}`)
    return data
  } catch {
    return {}
  }
 ...
 */
 export const Star = () : JSX.Element => {
  ...
  useEffect(() => {
    getRepo().then(({ stargazers_count }) => setStars(isNaN(Number(stargazers_count)) ? 0 : Number(stargazers_count) ))
  }, [])

  return <a href={`https://github.com/${PROJECT_PATH}`} target="_blank" className='px-3 py-1 bg-slate-300 dark:bg-slate-900 rounded-md border dark:border-slate-700 text-slate-600 dark:text-slate-300' rel="noreferrer">
